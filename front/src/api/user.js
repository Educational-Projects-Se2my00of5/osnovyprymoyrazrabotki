...
import { UserInfoDto, UpdateUserRequestDto, PageDto } from '../dto/userDto';
...
export const getAllUsers = async (page = 0, size = 20) => {
  try {
    const response = await apiClient.get('/user/all', { params: { page, size } });
    return PageDto(response.data, UserInfoDto);
  } catch (err) {
    const { message } = normalizeAxiosError(err);
    throw new Error(message);
  }
